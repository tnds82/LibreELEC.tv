...
enum dmx_source_front_t {	DMX_SOURCE_FRONT0_OFFSET = 100,
...
pr_dbg_irq("DUPLICATED_PACKET\n");
	}
	if (status & (1 << DIS_CONTINUITY_PACKET)) {
pr_dbg_irq("DIS_CONTINUITY_PACKET\n");
	}
...
static int dmx_init(struct aml_dmx *dmx)
{
	...
	if (dmx->init) {
pr_inf("demux already inited irq:%d\n", dmx->dmx_irq);
		return 0;
	}
	pr_inf("demux init irq:%d\n", dmx->dmx_irq);
	...
	if (dmx->dmx_irq != -1) {
		...
		irq = request_irq(dmx->dmx_irq,	dmx_irq_handler,
				IRQF_SHARED|IRQF_TRIGGER_RISING,
				"dmx irq", dmx);
printk("request irq:%d, id:%d, dmx:%p\n", irq, dmx->id, dmx);
	}
